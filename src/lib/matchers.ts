...
import { DaydreamDefaultError } from "../models/errors/daydreamdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | DaydreamDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | DaydreamDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new DaydreamDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new DaydreamDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
